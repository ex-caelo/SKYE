...
`CLAUDE.md` (conventions + status) and `SKYE-pre-scaffold-TODO.md`
...
Two packages:
- **`packages/skye-config`** — framework-agnostic: the JSON Schema, TS
...
- **Permission overlays are additive-only.** A higher-permission overlay
  ...
  by `@skye/config`'s `mergeConfig`/`lintOverlay`.
...
- **`/switcher` is a two-step chooser**, not just a site picker: with no
  ...
  there. See `scripts/entry-switcher.ts` and `lib/routing/siteSwitcher.ts`'s
...
139 tests passing (45 in `@skye/config`, 94 in `@skye/app`), both packages
...
- This repo, at minimum `CLAUDE.md`, `SKYE-pre-scaffold-TODO.md`, and the
