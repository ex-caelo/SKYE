...
- [ ] **(New)** MSAL redirect-fallback state recovery (§4): a popup-blocked user gets bounced through a full-page redirect, but nothing currently restores the SPA's hash/in-progress form state on return. Needs a design (e.g. stash `location.hash` + draft values in `sessionStorage` before `loginRedirect`, restore in `entry-form.ts` on load) before this is production-safe.
...
- [ ] **(New)** `PUBLIC_DEFAULT_APPLICATION_ID` (and now `PUBLIC_DEFAULT_TENANT_ID`, added alongside it — see §4) need to actually be set in your deployment for the site switcher to work when a URL arrives with no `applicationId`/`tenantId` at all. Not yet added to any `.env.example` or Astro env typing — just referenced via `import.meta.env` in `entry-switcher.ts`. Small doc/config gap, not a code gap.
...
- [x] **`pages/builder.astro` + `scripts/entry-builder.ts`** — the
  ...
  by hand-entering a form id + target list GUID (no new "list every
  SharePoint list on a site" Graph call needed — confirmed with the user
  as an acceptable scope cut); (3) the builder itself — a view switcher
