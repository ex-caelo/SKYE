 ...
 * skeleton components are expression-free, so everything after the `---`
 * frontmatter fence is usable HTML). This keeps the tests running against
 * the SAME markup the page ships — no hand-maintained fixture copy to
 * drift out of sync — without needing Astro's build pipeline in vitest.
 * Every reusable component lives in `packages/app/src/components/`.
 */
