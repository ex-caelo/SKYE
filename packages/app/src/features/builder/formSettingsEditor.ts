...
export interface FormSettingsEditorOptions {
   ...
   * `src/integrations/registry.ts`. When provided, a `script` postAction's
...
function renderMissingRequiredPanel(
  ...
  const addFieldForColumn = (fields: Record<string, unknown>, column: GraphListColumn, taken: Set<string>): void => {
    ...
    taken.add(key);
    // Land after every existing field so it stacks at the end of the form (matches the
    // single-column layout a builder-created form defaults to).
    const maxOrder = Math.max(0, ...Object.values(fields).map((f) => Number((f as { order?: unknown }).order) || 0));
    fields[key] = { ...fieldConfigForColumn(column, defaultPageKey), order: maxOrder + 1 };
  };
