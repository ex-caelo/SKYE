import type { FormConfig, FormConfigOverlay } from "@skye/form-config";
import {
  mergeConfig,
  lintOverlay,
  validateFormConfig,
...
import { renderFormSettingsEditor } from "../features/builder/formSettingsEditor.js";
...
import { controlTypeForColumn, fieldConfigForColumn, fieldKeyForColumn, requiredColumnFields, SINGLE_COLUMN_LAYOUT } from "../features/builder/columnMapping.js";
...
import { showState, el, fillSlot } from "../shared/ui/pageState.js";
...
async function openBuilder(
  ...
  if (isNew && newFormSeed) {
    ...
    // `layout` defaults to a single CSS Grid column so the auto-added required fields (below,
    // once the list's columns load) stack one per row in `order`.
  ...
  // actually submit from the start; with the single-column `layout` above they stack one per row.
  // (An existing form gets a "missing required columns" prompt instead — see
  // renderFormSettingsEditor — rather than a silent mutation of what's already saved.)
  if (isNew) {
    const base = state.views.get("base") as unknown as FormConfig;
    const fields = (base.fields ??= {}) as Record<string, unknown>;
    Object.assign(fields, requiredColumnFields(state.listColumns, Object.keys(base.pages ?? {})[0]));
  }
