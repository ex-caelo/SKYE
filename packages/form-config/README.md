...
| Actions | `src/post-actions/*` | `templating.ts` (`{{fields.x}}`/`{{item.x}}`/`{{results.x.y}}` interpolation), `dependencyGraph.ts` (topological batching + skip-cascade), `actionRunner.ts` (runs one trigger phase), `defaultHandlerRegistry.ts` + `handlers/*` (one file per postAction `type` — the extension point for new types). |
...
Run from the repo root (or with `--filter @skye/form-config` from anywhere in the workspace):
